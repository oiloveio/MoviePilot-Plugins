...
## v0.7.0 更新：strm 存放方式可选（平铺 / 按番剧名称聚合）

### 新增配置项：strm存放方式

- **平铺**（默认）：所有 strm 直接放在存储目录下
- **按番剧名称聚合**：每部剧一个文件夹，`storageplace/{剧名}/{完整文件名}.strm`

Emby/Jellyfin 侧一个文件夹对应一个剧集条目，刮削结果更干净。多季度的剧（比如"关于我转生变成史莱姆这档事 第四季"）季度信息本来就在标题里，会自然分到不同文件夹，**不需要额外套 `Season 01` 子目录**。

### 剧名解析：一份规则，一个函数

新增 `SERIES_TITLE_RE` + `StrmFileService.extract_series_title()`。ANi 命名格式固定是 `[ANi] 剧名 - 集数 [1080P][Baha]...`，剧名就是发布组标签之后、` - 集数 [` 之前那一段。两个真实数据里踩到的点：

1. **集数不一定是整数**：实测有 `- 12.5 [`（半集）和 `- 電影 [`（剧场版），所以这里**不能复用只认整数的 `EPISODE_NUM_RE`**，用的是更宽的 `- 任意非方括号内容 [`——这几个边缘文件被主正则直接覆盖，不需要单独的 fallback 分支
2. **剧名本身可能含 ` - `**（比如 `Fate - Grand Order`）：剧名部分用**贪婪**匹配，取最后一个 ` - xxx [` 当分隔点，不会把剧名截成前半截

真正解析不出来的（完全不符合 ANi 命名格式）返回 `None`，这种文件平铺到根目录，不瞎猜目录名。

剧名转目录名只做最小限度处理（挡住路径分隔符防止建出多层目录）——真实数据（2650 个文件）核对过没有文件系统非法字符、没有超长文件名，不做过度清洗。

### 只有一处在决定目录，另外三个任务天然不受影响

原本担心"拉新番 / 回溯补集数 / 换源修复"三处各自解析一遍剧名、以后规则一改漏改一处会出现"同一部剧一半在文件夹里一半在根目录"的分裂。实际核对代码后确认只需要改一处：

| 任务 | 怎么决定文件位置 | 要不要改 |
|---|---|---|
| 拉新番 `__task` | 唯一决定新文件目录的地方 | ✅ 改成调 `__resolve_relative_dir()` |
| 资源补齐 `__backfill_task` | `ref_file.with_name()`，天然跟参照文件同目录 | ❌ 不用改，本来就正确 |
| 刷新订阅源 `__refresh_subscription_task` | `strm_file.write_text()` 原地改内容 | ❌ 不挪位置 |
| 套用加速源 `__apply_accelerator_task` | 同上，原地改内容 | ❌ 不挪位置 |

给后两类任务硬加一遍剧名解析反而制造了重复实现——正是要避免的那种分裂来源。所以解析逻辑只有 `extract_series_title()` 这一份，目录决策只有 `__resolve_relative_dir()` 这一处。

### 存量迁移：一键重新归档

新增「按存放方式重新归档本地strm」开关（`__regroup_local_strm_task`）：按当前选的存放方式把已有文件搬到对应位置。双向都支持——平铺→按剧名聚合、按剧名聚合→平铺，也能把历史版本留下的**季度目录**（`2026-7/` 这种）里的文件收进剧名文件夹。

- 只移动文件，不改内容，**不发任何网络请求**（改存放方式是纯本地整理，跟链接能不能连通是两回事）
- 目标位置已有同名文件时保留原文件不动，不覆盖
- 解析不出剧名的平铺到根目录（不会被留在旧的季度目录里漏掉）
- 搬完自底向上清理空目录

### 顺带移除：按季度月份分子目录

0.6.0 把"按季度分子目录"定成了固定行为（`2026-7/` 这种按 ANi 收录月份分的目录）。这跟"按番剧名称聚合"是互斥的两套组织方式，而且对媒体库有害：同一部剧的集数会跨月份被拆到不同文件夹（资源补齐那次多季度回溯已经证实过剧集确实跨月份）。这一版整个去掉，目录结构只由「strm存放方式」这一个选项决定。

## v0.6.0 更新：复杂度精简 + 正确性修复

版本号从这一版起改用 `0.x.y` 前缀（之前的 `3.x`/`4.0.0`/`5.0.0` 按同样的位数规则统一改写成 `0.3.x`/`0.4.0`/`0.5.0`，含义不变，只是不再让版本号看起来比实际成熟度更"正式"）。

用户对 0.5.0 做了一轮扎实的复盘，指出复杂度和核心需求不匹配、以及两个真实的正确性缺口，这一版逐条解决：

### 1. 订阅源/加速源进一步简化：从"列表+单选生效"改成各填一个值

0.5.0 把订阅源和加速源都改成了"候选列表 + 当前生效选择器"的对称模型，用来解决 0.4.0 的坏链接问题。但复盘指出：核心需求只是"抓新番建 strm、保证能播"，用户要理解"订阅源列表""生效订阅源""目标订阅源"这几个概念，是给架构债务打补丁带出来的认知负担，不是任务本身需要的。

这一版直接砍掉"列表"这个概念：`subscription_source`/`accelerator_prefix` 各自只是一个字符串字段。多源容灾没有消失，而是挪到后台自动处理——`FALLBACK_SUBSCRIPTION_POOL` 是内置的几个已知镜像，用户配置的订阅源抓取失败时按顺序自动依次尝试，全程只对用户暴露"抓取失败了，自动换了一个"这一句日志，不需要用户理解"列表"或"选择生效项"。加速源同理：每次拉新番前用当次样本直链实测一次"这个加速源能不能用"，测不通就当次任务自动不加速、写裸直链，不阻塞整个任务——这一次性预检查从设计上避免了"加速源套在一个连不通的组合上批量生成坏链接"，比 0.4.0"完全不测"和 0.5.0"逐条测"这两个极端都更合适：只测一次，不拖慢拉取速度，又能兜住这次真正踩到的那类问题。

"本地strm维护"也跟着简化：`refresh_subscription_once`/`apply_accelerator_once` 两个开关，直接用"当前配置的这一个值"操作全部本地 strm，不再需要"目标订阅源"/"目标加速源"这种额外的选择器概念。还原/切换/套壳都统一走 `extract_resource_path()` 定位跟域名无关的"季度/文件名?query"这一段、配上目标域名重建——不需要"记住"strm 当初到底被套过哪个加速源。

### 2. 连通性探测补上内容校验，不能只看 HTTP 状态码

`probe_latency_ms()` 之前只判定 `status_code in (200, 206)`，没检查响应到底是不是真的视频数据。真实排查中确认过：服务器完全可能返回 200/206，但吐的是一个错误页（HTML/JSON），单看状态码会把这种情况误判成"可达"。

这一版探测请求把 Range 从 1 字节扩大到 64 字节（`PROBE_RANGE_BYTES`），足够读到 mp4 容器 `ftyp` box 的魔数字节，同时校验 `Content-Type` 不是 `text/html`/`application/json` 这类网页格式、内容开头不是明显的错误页标记（`<!doctype`/`<html`/`<?xml`/`{`）。两层都通过才认定"真的可达"，任何一层没通过都返回明确的失败原因，不再把"内容不对"误判成"可达"。

### 3. 资源补齐支持同时尝试多个季度文件夹，不再一碰边界就弃剧

之前的实现只在参照文件所在的季度目录里递减集数，一旦某一集探测不通就整部剧直接停止回溯。用户实测多部剧确认过：季度目录是按剧集**首次收录**的月份命名，不是按每一集的实际上传日期——一拳超人第三季真实起点落在第 25 集、SPY×FAMILY 第三季真实起点落在第 38 集，都在比本地当前最早一集更靠前的季度文件夹里。按旧逻辑，只要本地最早集恰好卡在季度边界，继续往前的集数会被直接漏掉。

这一版新增 `build_season_variant_link()`/`season_distance()`：扫描本地全部 strm 收集"已知季度文件夹"集合，探测某一集时先试当前季度，不通就按跟当前季度的月份距离由近到远，尝试其它已知季度文件夹（每集最多尝试 4 个，`max_season_candidates_per_episode`），全部试过还是不通才停止继续往前探测这部剧；命中的季度会记成下一集的优先候选（大概率连续几集在同一个文件夹里，减少后续探测次数）。

### 4. 移除 Relay 转发的规划描述，改为文档推荐自建反代

0.5.0 已经把 Relay 转发功能整体移除，但 README 里还留了"以后做本地局域网代理时重新设计成加速源类型"这句规划。复盘指出：这类"网络层转发"本就该交给专业工具，`加速源` 现有的"前缀拼接"能力已经跟自建反代完全兼容——用户可以自己用 GOST/Nginx 等工具搭一个"Proxy Everything"风格的反向代理，把反代地址直接当成 `accelerator_prefix` 填进去，跟 pili/op5 这类社区镜像用法完全一样，插件不需要任何额外代码支持。这一版去掉了"以后再做"的规划表述，配置页和使用说明改成直接推荐这个方案。

### 5. 精简锦上添花的配置项

- 移除 `filename_remove`（文件名删除字符串）：ANi 官方直链的标题本身就是干净的（`[ANi] 标题 - 集数 [分辨率]...`），这个配置项是借鉴 ANiStrmPro 时顺带引入的，对 ANi 自己的订阅源实际用不上
- `filename_blacklist` 不再是用户配置项，固定为常见非正片关键词常量 `NON_EPISODE_BLACKLIST = "预告@PV@NCOP@NCED"`，始终生效
- `season_dir`（按季度分子目录存放）不再是开关，固定为始终开启——这跟插件推荐的目录监控用法本来就是一致的，没有理由让用户关掉

配置面从"核心设置 + 生成规则(可选) + 多张维护卡片"精简成"核心设置 + 订阅源/加速源两个字段 + 三张维护卡片"，减少用户需要理解的概念数量。

## v0.5.0 更新：架构重构——订阅源/加速源改成对称的"列表+单选生效"模型

### 起因：一条真实坏链接暴露出的架构问题

0.4.0 上线后实测出一条坏链接：`https://pro.pili.cc.cd/ani.td.ee/2025-10/...`——加速源前缀 `pili` 被套在了另一个域名 `ani.td.ee`（本 README 里明确记录过的已知限流失效镜像）外面，拼出一个谁都处理不了的双层地址。

排查根因发现这不是孤立 bug，而是**架构本身的问题**：0.4.0 的"订阅源"是多个 RSS 镜像同时启用、按标题去重、谁先抓到内容就用谁的直链（"多源聚合"模型）；"加速源"是后来单独加的一套"列表 + 选中生效"模型。这两套机制互不知道对方状态——加速源套壳时，完全不知道这条直链究竟来自哪个数据源、那个数据源现在健不健康，于是"一个本该被 `#` 禁用的失效源意外生效，贡献了内容，又被自动套上一层不相关的加速前缀"这种组合，系统设计上根本没有被排除掉。类似"意外启用失效源"的问题在 v0.3.6/v0.3.7 已经出现过一次，这次是它的又一次变种。
...
### 新增：本地 strm 维护（合并掉 0.4.0 的四个独立任务）
...
对每个未禁用的订阅源取一条样本直链，测"直连"，再测"套上每个未禁用的加速源"之后的连通情况。行 = 订阅源，列 = [直连, 加速源1, 加速源2, ...]。这张矩阵直接回答"该选哪个订阅源 + 哪个加速源组合"——用户看一眼矩阵就知道某个源/某个组合连不通，不用等生成出坏链接才发现。取代 0.4.0 分开的"数据源健康度"+"加速源健康度"两张表，同时统计本地 strm 按"订阅源+加速源"分类的分布，取代只按域名统计的旧版分布表。
...
0.4.0 的 Relay 转发（`RelayService`、`get_api()` 注册的 `/relay` 路由、配置页的独立开关）整体移除。这是一次性破坏性重构，`get_api()` 恢复成 `return []`，插件重新回到只依赖标准库 + apscheduler/pytz 的轻量状态。
...
## v0.4.0 更新：加速源体系完善 + 资源补齐 + Relay转发(实验性，0.5.0已移除)

### 加速源体系完善

v0.3.8 的「套代理前缀」发布后发现一个真实遗漏：`proxy_prefix` 只在「事后批量套壳」（`__apply_proxy_prefix_task`）里生效，拉新番的主任务（`__task`）压根没用上——配了代理前缀，新拉的番还是裸链接，得手动再跑一次套壳。这一版修：
...
装了v0.3.6之后用户反馈修复链接**仍然**大量"候选链接探测不可达"，怀疑是不是插件没有文件写权限。排查过程：
...
- 真正原因：v0.3.6虽然改成了"探测挑一个健康源当参照"，但一次修复任务要处理几十上百个文件、跑好几分钟，全程只认定**唯一一个**参照源——这个源中途只要被限流/网络抖动一下，后面所有文件就会一起失败，表现成一长串"不可达"，很容易被误判成别的问题。而且原来的日志只写死"不可达"三个字，看不出究竟是真死了、纯超时、还是被限流，没法排查（这条本身就是设计缺陷）。
...
本插件原名 ANi-Strm，是 honue 原版的 fork。为避免和 honue 原版及其他 fork（ANiStrmPlus、ANiStrmPro 等）撞插件 ID，从 v0.3.4 起插件 ID/类名/目录名统一改为 **ANiStrmHub**。已安装旧版 ANiStrm 的需要按新 ID 重新安装（视为不同插件，配置不会自动迁移）。
